...
import { useAllStaff } from '@/lib/queries/staff';
import { useStaffFilterStore } from '@/lib/store/staffFilterStore';
...
export default function StaffListingPage() {
  ...
  const {
    ...
    setSearchQuery, setSelectedDepartment, setSelectedDesignation, clearFilters,
  } = useStaffFilterStore();

  const departments  = useMemo(() => [...new Set(staff.map((s) => s.department?.name).filter(Boolean))], [staff]);
  const designations = useMemo(() => [...new Set(staff.map((s) => s.designation))], [staff]);
  ...
  const filteredStaff = useMemo(() => {
    ...
    if (selectedDepartment !== 'all') list = list.filter((s) => s.department?.name === selectedDepartment);
    if (selectedDesignation !== 'all') list = list.filter((s) => s.designation === selectedDesignation);
    return list;
  }, [staff, searchQuery, selectedDepartment, selectedDesignation]);

  const totalPublications = staff.reduce((sum, s) => sum + (s._count?.publications ?? 0), 0);
          ...
          <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-6">
            <div>
              <div className="flex items-center gap-3 mb-3">
                ...
                <span className="text-white/70 text-sm font-medium tracking-widest uppercase">MNSUAM</span>
              </div>
              <h1 className="text-4xl font-bold tracking-tight mb-2">Faculty Members</h1>
              <p className="text-white/70 text-lg">Meet our distinguished academic community</p>
            </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-16">
        {filteredStaff.length === 0 ? (
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 py-20 text-center">
            ...
            <p className="text-gray-500 text-sm mb-6">No results match your current filters.</p>
            {hasActiveFilters && (
              <button
                onClick={clearFilters}
              ...
              >
                <div className="p-6 flex items-start gap-4 flex-1">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-sm font-medium text-[#c9a961] mt-0.5 truncate">{member.designation}</p>
                ...
                <div className="mx-6 mb-4 border border-gray-100 rounded-xl grid grid-cols-2 divide-x divide-gray-100">
                  <div className="flex items-center gap-2 px-4 py-3">
                    ...
                    <div>
                      <p className="text-sm font-bold text-gray-900">{member._count?.publications ?? 0}</p>
                  ...
                  <div className="flex items-center gap-2 px-4 py-3">
                    ...
                    <div>
                      <p className="text-sm font-bold text-gray-900">{member._count?.projects ?? 0}</p>
